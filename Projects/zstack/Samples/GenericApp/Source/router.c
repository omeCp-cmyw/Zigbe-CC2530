...
void GenericApp_Init( uint8 task_id )
{
  ...
  GenericApp_DstAddr.endPoint =10;//发到协调器的10号端点
...
static void GenericApp_SendTheMessage( void )
{
   unsigned char theMessageData[10];
  DHT11();//温湿度 值
 theMessageData[0]='R';
 theMessageData[1]=wendu_shi+0x30;
 theMessageData[2]=wendu_ge+0x30; 
 theMessageData[3]=shidu_shi+0x30;
 theMessageData[4]=shidu_ge+0x30;
 theMessageData[5]=0;

  if ( AF_DataRequest( &GenericApp_DstAddr, &GenericApp_epDesc,//发送目标地址以及源地址
                       GENERICAPP_ROT,//需要指定簇，有簇对应才能够对应节点数据
